...
public class FarmlandWaterManager
{
    ...
    public static<T extends SimpleTicket<Vec3d>> T addCustomTicket(World world, T ticket, ChunkPos masterChunk, ChunkPos... additionalChunks)
    {
        ...
        Map<ChunkPos, ChunkTicketManager<Vec3d>> ticketMap = customWaterHandler.computeIfAbsent(world.func_201675_m().func_186058_p().func_186068_a(), id -> new MapMaker().weakValues().makeMap());
    ...
    public static AABBTicket addAABBTicket(World world, AxisAlignedBB aabb)
    {
        ...
        {
            double distToCenter = getDistanceSq(pos, aabb.func_189972_c());
    ...
    {
        ChunkTicketManager<Vec3d> ticketManager = getTicketManager(chunk.func_76632_l(), chunk.getWorldForge());
        if (ticketManager != null)
        {
            if (DEBUG)
    ...
    private static ChunkTicketManager<Vec3d> getTicketManager(ChunkPos pos, IWorldReader world) {
        Preconditions.checkArgument(!world.func_201670_d(), "Water region is only determined server-side");
        Map<ChunkPos, ChunkTicketManager<Vec3d>> ticketMap = customWaterHandler.get(world.func_201675_m().func_186058_p().func_186068_a());
        if (ticketMap == null)
        {
            return null;
        }
