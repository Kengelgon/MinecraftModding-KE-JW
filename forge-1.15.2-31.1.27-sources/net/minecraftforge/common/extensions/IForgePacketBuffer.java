...
public interface IForgePacketBuffer
{
    ...
    {
        ForgeRegistry<T> forgeRegistry = (ForgeRegistry<T>) registry;
        int id = getBuffer().func_150792_a();
    ...
    default <T extends IForgeRegistryEntry<T>> void writeRegistryId(@Nonnull T entry)
    {
        ...
        getBuffer().func_192572_a(name); //TODO change to writing a varInt once registries use id's
        getBuffer().func_150787_b(reg.getID(entry));
    }
    ...
    {
        ResourceLocation location = getBuffer().func_192575_l(); //TODO change to reading a varInt once registries use id's
        ForgeRegistry<T> registry = RegistryManager.ACTIVE.getRegistry(location);
        return registry.getValue(getBuffer().func_150792_a());
    }
