...
public class ForgeEventFactory
{
    ...
    public static boolean canEntitySpawnSpawner(MobEntity entity, World world, float x, float y, float z, AbstractSpawner spawner)
    {
        ...
        if (result == Result.DEFAULT)
            return entity.func_213380_a(world, SpawnReason.SPAWNER) && entity.func_205019_a(world); // vanilla logic (inverted)
        else
    ...
    public static int onHoeUse(ItemUseContext context)
    {
        ...
        {
            context.func_195996_i().func_222118_a(1, context.func_195999_j(), player -> player.func_213334_d(context.func_221531_n()));
            return 1;
        }
        return 0;
    }
    ...
    public static boolean canMountEntity(Entity entityMounting, Entity entityBeingMounted, boolean isMounting)
    {
        ...
        if(isCanceled)
        {
            entityMounting.func_70080_a(entityMounting.func_226277_ct_(), entityMounting.func_226278_cu_(), entityMounting.func_226281_cx_(), entityMounting.field_70126_B, entityMounting.field_70127_C);
            return false;
        }
        else
            return true;
    ...
    {
        return renderBlockOverlay(player, mat, OverlayType.FIRE, Blocks.field_150480_ab.func_176223_P(), new BlockPos(player));
    }

    @OnlyIn(Dist.CLIENT)
    public static boolean renderWaterOverlay(PlayerEntity player, MatrixStack mat)
    {
        return renderBlockOverlay(player, mat, OverlayType.WATER, Blocks.field_150355_j.func_176223_P(), new BlockPos(player));
    }
    ...
    public static boolean fireSleepingLocationCheck(LivingEntity player, BlockPos sleepingLocation)
    {
        ...
        if (canContinueSleep == Result.DEFAULT)
        {
            return player.func_213374_dv().map(pos-> {
                ...
                return state.func_177230_c().isBed(state, player.field_70170_p, pos, player);
            }).orElse(false);
        }
        else
    ...
    public static boolean getMobGriefingEvent(World world, Entity entity)
    {
        ...
        return result == Result.DEFAULT ? world.func_82736_K().func_223586_b(GameRules.field_223599_b) : result == Result.ALLOW;
    }
