...
public final class DynamicBucketModel implements IModelGeometry<DynamicBucketModel>
{
    ...
    public IBakedModel bake(IModelConfiguration owner, ModelBakery bakery, Function<Material, TextureAtlasSprite> spriteGetter, IModelTransform modelTransform, ItemOverrideList overrides, ResourceLocation modelLocation)
    {
        ...
        if (MissingTextureSprite.func_195675_b().equals(particleLocation.func_229313_b_()))
        {
            particleLocation = null;
        }

        Material baseLocation = owner.resolveTexture("base");
        if (MissingTextureSprite.func_195675_b().equals(baseLocation.func_229313_b_()))
        {
            baseLocation = null;
        }

        Material fluidMaskLocation = owner.resolveTexture("fluid");
        if (MissingTextureSprite.func_195675_b().equals(fluidMaskLocation.func_229313_b_()))
        {
            fluidMaskLocation = null;
        }

        Material coverLocation = owner.resolveTexture("cover");
        if (!MissingTextureSprite.func_195675_b().equals(coverLocation.func_229313_b_()))
        {
        ...
        if (flipGas && fluid != Fluids.field_204541_a && fluid.getAttributes().isLighterThanAir())
        {
        ...
        TextureAtlasSprite fluidSprite = fluid != Fluids.field_204541_a ? spriteGetter.apply(ForgeHooksClient.getBlockMaterial(fluid.getAttributes().getStillTexture())) : null;
            ...
            return FluidUtil.getFluidContained(stack)
                    .map(fluidStack -> {
                        ...
                        if (!model.cache.containsKey(name))
                        {
                            ...
                            IBakedModel bakedModel = parent.bake(model.owner, bakery, ModelLoader.defaultTextureGetter(), model.originalTransform, model.func_188617_f(), new ResourceLocation("forge:bucket_override"));
