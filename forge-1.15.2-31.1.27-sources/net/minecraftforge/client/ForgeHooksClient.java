...
//        ModelLoader.White.INSTANCE.register(map); // TODO Custom TAS
    ...
    {
        A model = itemStack.func_77973_b().getArmorModel(entityLiving, itemStack, slot, _default);
...
*/
    ...
    public static int getSkyBlendColour(World world, BlockPos center)
    {
        ...
        for (int x = -distance; x <= distance; ++x)
        {
            for (int z = -distance; z <= distance; ++z)
            {
                ...
                Biome biome = world.func_225526_b_(pos.func_177958_n(), pos.func_177956_o(), pos.func_177952_p());
    ...
    public static void renderMainMenu(MainMenuScreen gui, FontRenderer font, int width, int height)
    {
        ...
        if (status == BETA || status == BETA_OUTDATED)
        {
            ...
            gui.drawString(font, line, (width - font.func_78256_a(line)) / 2, 4 + (0 * (font.field_78288_b + 1)), -1);
            line = I18n.func_135052_a("forge.update.beta.2");
            gui.drawString(font, line, (width - font.func_78256_a(line)) / 2, 4 + (1 * (font.field_78288_b + 1)), -1);
        }
    ...
    {
        EntityViewRenderEvent.CameraSetup event = new EntityViewRenderEvent.CameraSetup(renderer, info, partial, info.func_216778_f(), info.func_216777_e(), 0);
        MinecraftForge.EVENT_BUS.post(event);
        return event;
    }
    ...
    public static void preDraw(Usage attrType, VertexFormat format, int element, int stride, ByteBuffer buffer)
    {
        ...
        switch(attrType)
        {
            ...
            case COLOR:
                ...
                break;
            case UV:
                GL13.glClientActiveTexture(GL13.GL_TEXTURE0 + attr.func_177369_e());
            ...
            case GENERIC:
                ...
                glVertexAttribPointer(attr.func_177369_e(), count, constant, false, stride, buffer);
                break;
            default:
                LOGGER.fatal("Unimplemented vanilla attribute upload: {}", attrType.func_177384_a());
        }
    }
    ...
    public static int getColorIndex(VertexFormat fmt)
    {
        ...
        {
            if (elements.get(i).func_177375_c() == Usage.COLOR)
                return i;
        }
    ...
    public static TextureAtlasSprite[] getFluidSprites(ILightReader world, BlockPos pos, IFluidState fluidStateIn)
    {
        ...
        return new TextureAtlasSprite[] {
                Minecraft.func_71410_x().func_228015_a_(AtlasTexture.field_110575_b).apply(fluidStateIn.func_206886_c().getAttributes().getStillTexture(world, pos)),
                Minecraft.func_71410_x().func_228015_a_(AtlasTexture.field_110575_b).apply(fluidStateIn.func_206886_c().getAttributes().getFlowingTexture(world, pos)),
                overlayTexture == null ? null : Minecraft.func_71410_x().func_228015_a_(AtlasTexture.field_110575_b).apply(overlayTexture),
        };
    }
    ...
    {
        return new Material(AtlasTexture.field_110575_b, loc);
    }
    ...
    public static void fillNormal(int[] faceData, Direction facing)
    {
        ...
        int x = ((byte) Math.round(v2.func_195899_a() * 127)) & 0xFF;
        int y = ((byte) Math.round(v2.func_195900_b() * 127)) & 0xFF;
        int z = ((byte) Math.round(v2.func_195902_c() * 127)) & 0xFF;
    ...
    {
        Event event = new InputEvent.MouseScrollEvent(scrollDelta, mouseHelper.func_198030_b(), mouseHelper.isMiddleDown(), mouseHelper.func_198031_d(), mouseHelper.func_198024_e(), mouseHelper.func_198026_f());
        return MinecraftForge.EVENT_BUS.post(event);
    }
