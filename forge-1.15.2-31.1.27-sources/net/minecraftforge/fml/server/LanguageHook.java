    ...
    private static void loadLocaleData(final InputStream inputstream) {
        try
        {
            ...
            jsonobject.entrySet().forEach(entry -> {
                String s = PATTERN.matcher(JSONUtils.func_151206_a(entry.getValue(), entry.getKey())).replaceAll("%$1s");
    ...
    private static void loadLanguage(String langName, MinecraftServer server) {
        ...
        server.func_195570_aG().func_199001_a().forEach(namespace -> {
            try {
                ...
                loadLocaleData(server.func_195570_aG().func_199004_b(langResource));
