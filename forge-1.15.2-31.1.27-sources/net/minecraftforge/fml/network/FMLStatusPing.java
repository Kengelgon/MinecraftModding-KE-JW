...
public class FMLStatusPing {
    ...
    public static class Serializer {
        public static FMLStatusPing deserialize(JsonObject forgeData, JsonDeserializationContext ctx) {
            try {
                final Map<ResourceLocation, Pair<String, Boolean>> channels = StreamSupport.stream(JSONUtils.func_151214_t(forgeData, "channels").spliterator(), false).
                        map(JsonElement::getAsJsonObject).
                        collect(Collectors.toMap(jo -> new ResourceLocation(JSONUtils.func_151200_h(jo, "res")),
                                jo -> Pair.of(JSONUtils.func_151200_h(jo, "version"), JSONUtils.func_151212_i(jo, "required")))
                        );

                final Map<String, String> mods = StreamSupport.stream(JSONUtils.func_151214_t(forgeData, "mods").spliterator(), false).
                        map(JsonElement::getAsJsonObject).
                        collect(Collectors.toMap(jo -> JSONUtils.func_151200_h(jo, "modId"), jo->JSONUtils.func_151200_h(jo, "modmarker")));

                final int remoteFMLVersion = JSONUtils.func_151203_m(forgeData, "fmlNetworkVersion");
