...
{
    private static String stripControlCodes(String value) { return net.minecraft.util.StringUtils.func_76338_a(value); }
    ...
    private enum SortType implements Comparator<ModInfo>
    {
        ...
        String getButtonText() {
            return I18n.func_135052_a("fml.menu.mods."+StringUtils.toLowerCase(name()));
        }
    }
    ...
    class InfoPanel extends ScrollPanel {
        ...
        private List<ITextComponent> resizeContent(List<String> lines)
        {
            ...
            for (String line : lines)
            {
                ...
                if (maxTextLength >= 0)
                {
                    ret.addAll(RenderComponentsUtil.func_178908_a(chat, maxTextLength, ModListScreen.this.font, false, true));
                }
            }
            return ret;
        }
        ...
        {
            int height = 50;
            height += (lines.size() * font.field_78288_b);
        ...
        {
            if (logoPath != null) {
                Minecraft.func_71410_x().func_110434_K().func_110577_a(logoPath);
            ...
            for (ITextComponent line : lines)
            {
                ...
                {
                    RenderSystem.enableBlend();
                    ModListScreen.this.font.func_175063_a(line.func_150254_d(), left + PADDING, relativeY, 0xFFFFFF);
                    RenderSystem.disableAlphaTest();
                    RenderSystem.disableBlend();
                }
                relativeY += font.field_78288_b;
            }
        ...
        private ITextComponent findTextLine(final int mouseX, final int mouseY) {
            ...
            int lineIdx = (int) (offset / font.field_78288_b);
            ...
            if (line != null)
            {
                ...
                for (ITextComponent part : line) {
                    if (!(part instanceof StringTextComponent))
                        continue;
                    k += ModListScreen.this.font.func_78256_a(((StringTextComponent)part).func_150265_g());
                    if (k >= mouseX)
                    {
                        return part;
                    }
    ...
    public void init()
    {
        ...
        {
            listWidth = Math.max(listWidth,getFontRenderer().func_78256_a(mod.getDisplayName()) + 10);
            listWidth = Math.max(listWidth,getFontRenderer().func_78256_a(MavenVersionStringHelper.artifactVersionToString(mod.getVersion())) + 5);
        }
        ...
        this.addButton(new Button(((listWidth + PADDING + this.width - doneButtonWidth) / 2), y, doneButtonWidth, 20,
                I18n.func_135052_a("gui.done"), b -> ModListScreen.this.minecraft.func_147108_a(ModListScreen.this.mainMenu)));
        this.addButton(this.openModsFolderButton = new Button(6, y, this.listWidth, 20,
                I18n.func_135052_a("fml.menu.mods.openmodsfolder"), b -> Util.func_110647_a().func_195641_a(FMLPaths.MODSDIR.get().toFile())));
        y -= 20 + PADDING;
        this.addButton(this.configButton = new Button(6, y, this.listWidth, 20,
                I18n.func_135052_a("fml.menu.mods.config"), b -> ModListScreen.this.displayModConfig()));
        this.configButton.active = false;

        y -= 14 + PADDING + 1;
        search = new TextFieldWidget(getFontRenderer(), PADDING + 1, y, listWidth - 2, 14, I18n.func_135052_a("fml.menu.mods.search"));

        int fullButtonHeight = PADDING + 20 + PADDING;
        this.modList = new ModListWidget(this, listWidth, fullButtonHeight, search.y - getFontRenderer().field_78288_b - PADDING);
    ...
    private void displayModConfig()
    {
        ...
        try
        {
            ConfigGuiHandler.getGuiFactoryFor(selected.getInfo()).map(f->f.apply(this.minecraft, this)).ifPresent(newScreen -> this.minecraft.func_147108_a(newScreen));
        }
    ...
    private void reloadMods()
    {
        this.mods = this.unsortedMods.stream().
                filter(mi->StringUtils.toLowerCase(stripControlCodes(mi.getDisplayName())).contains(StringUtils.toLowerCase(search.func_146179_b()))).collect(Collectors.toList());
    ...
    public void render(int mouseX, int mouseY, float partialTicks)
    {
        ...
        int x = modList.getLeft() + ((modList.getRight() - modList.getLeft()) / 2) - (getFontRenderer().func_78256_a(text) / 2);
        getFontRenderer().func_211126_b(text, x, search.y - getFontRenderer().field_78288_b, 0xFFFFFF);
    ...
    private void updateCache()
    {
        ...
        Pair<ResourceLocation, Size2i> logoData = selectedMod.getLogoFile().map(logoFile->
        {
            ...
            try
            {
                ...
                InputStream logoResource = resourcePack.func_195763_b(logoFile);
                if (logoResource != null)
                    logo = NativeImage.func_195713_a(logoResource);
                if (logo != null)
                {

                    return Pair.of(tm.func_110578_a("modlogo", new DynamicTexture(logo) {
                    ...
                    }), new Size2i(logo.func_195702_a(), logo.func_195714_b()));
                }
            }
            catch (IOException e) { }
