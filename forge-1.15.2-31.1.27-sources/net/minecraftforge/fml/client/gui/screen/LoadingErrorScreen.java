...
public class LoadingErrorScreen extends ErrorScreen {
    ...
    public void init()
    {
        ...
        this.addButton(new ExtendedButton(50, this.height - yOffset, this.width / 2 - 55, 20, ForgeI18n.parseMessage("fml.button.open.mods.folder"), b -> Util.func_110647_a().func_195641_a(modsDir.toFile())));
        this.addButton(new ExtendedButton(this.width / 2 + 5, this.height - yOffset, this.width / 2 - 55, 20, ForgeI18n.parseMessage("fml.button.open.file", logFile.getFileName()), b -> Util.func_110647_a().func_195641_a(logFile.toFile())));
    ...
    private void drawMultiLineCenteredString(FontRenderer fr, String str, int x, int y) {
        for (String s : fr.func_78271_c(str, this.width)) {
            fr.func_175063_a(s, (float) (x - fr.func_78256_a(s) / 2.0), y, 0xFFFFFF);
            y+=fr.field_78288_b;
        }
    }
    public static class LoadingEntryList extends ExtendedList<LoadingEntryList.LoadingMessageEntry> {
        LoadingEntryList(final LoadingErrorScreen parent, final List<ModLoadingException> errors, final List<ModLoadingWarning> warnings) {
            super(parent.minecraft, parent.width, parent.height, 35, parent.height - 50, 2 * parent.minecraft.field_71466_p.field_78288_b + 8);
            ...
            if (both) {
                int maxChars = (this.width - 10) / parent.minecraft.field_71466_p.func_78256_a("-");
        ...
        public class LoadingMessageEntry extends ExtendedList.AbstractListEntry<LoadingMessageEntry> {
            ...
            public void render(int entryIdx, int top, int left, final int entryWidth, final int entryHeight, final int mouseX, final int mouseY, final boolean p_194999_5_, final float partialTicks) {
                ...
                final List<String> strings = font.func_78271_c(message, LoadingEntryList.this.width);
                int y = top + 2;
                for (int i = 0; i < Math.min(strings.size(), 2); i++) {
                    if (center)
                        font.func_211126_b(strings.get(i), left + (width  / 2F) - font.func_78256_a(strings.get(i)) / 2F, y, 0xFFFFFF);
                    else
                        font.func_211126_b(strings.get(i), left + 5, y, 0xFFFFFF);
