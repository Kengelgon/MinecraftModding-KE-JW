...
public class GuiUtils
{
    ...
    public static void drawTexturedModalRect(int x, int y, int u, int v, int width, int height, float zLevel)
    {
        ...
        wr.func_225582_a_(x        , y + height, zLevel).func_225583_a_( u          * uScale, ((v + height) * vScale)).func_181675_d();
        wr.func_225582_a_(x + width, y + height, zLevel).func_225583_a_((u + width) * uScale, ((v + height) * vScale)).func_181675_d();
        wr.func_225582_a_(x + width, y         , zLevel).func_225583_a_((u + width) * uScale, ( v           * vScale)).func_181675_d();
        wr.func_225582_a_(x        , y         , zLevel).func_225583_a_( u          * uScale, ( v           * vScale)).func_181675_d();
    ...
    public static void drawHoveringText(@Nonnull final ItemStack stack, List<String> textLines, int mouseX, int mouseY, int screenWidth, int screenHeight, int maxTextWidth, FontRenderer font)
    {
        if (!textLines.isEmpty())
        {
            ...
            IRenderTypeBuffer.Impl renderType = IRenderTypeBuffer.func_228455_a_(Tessellator.func_178181_a().func_178180_c());
