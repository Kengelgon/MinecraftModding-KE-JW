...
public class EarlyLoaderGUI {
    ...
    public EarlyLoaderGUI(final MainWindow window) {
        ...
        RenderSystem.clear(GL11.GL_COLOR_BUFFER_BIT, Minecraft.field_142025_a);
    ...
    private void setupMatrix() {
        ...
        RenderSystem.ortho(0.0D, window.func_198109_k() / window.func_198100_s(), window.func_198091_l() / window.func_198100_s(), 0.0D, 1000.0D, 3000.0D);
    ...
    private void renderMessages() {
        ...
        for (int i = 0; i < messages.size(); i++) {
            ...
            final float fade = MathHelper.func_76131_a((4000.0f - (float) pair.getLeft() - ( i - 4 ) * 1000.0f) / 5000.0f, 0.0f, 1.0f);
            if (fade <0.01f) continue;
            StartupMessageManager.Message msg = pair.getRight();
            renderMessage(msg.getText(), msg.getTypeColour(), ((window.func_198087_p() - 15) / 10) - i + 1, fade);
        }
        renderMemoryInfo();
    }
    ...
    private void renderMemoryInfo() {
        ...
        final int i = MathHelper.func_181758_c((1.0f - (float)Math.pow(pctmemory, 1.5f)) / 3f, 1.0f, 0.5f);
    ...
    void renderMessage(final String message, final float[] colour, int line, float alpha) {
        ...
        GlStateManager.func_227772_z_(GL11.GL_VERTEX_ARRAY);
