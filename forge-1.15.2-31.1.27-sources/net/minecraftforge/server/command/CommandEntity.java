...
class CommandEntity
{
    ...
    private static class EntityListCommand
    {
        ...
        static ArgumentBuilder<CommandSource, ?> register()
        {
            return Commands.func_197057_a("list")
                ...
                .then(Commands.func_197056_a("filter", StringArgumentType.string())
                    ...
                    .executes(ctx -> execute(ctx.getSource(), StringArgumentType.getString(ctx, "filter"), ctx.getSource().func_197023_e().field_73011_w.func_186058_p()))
                )
                .executes(ctx -> execute(ctx.getSource(), "*", ctx.getSource().func_197023_e().field_73011_w.func_186058_p()));
        }
        ...
        private static int execute(CommandSource sender, String filter, DimensionType dim) throws CommandSyntaxException
        {
            ...
            ServerWorld world = DimensionManager.getWorld(sender.func_197028_i(), dim, false, false);
            if (world == null)
            ...
            if (names.size() == 1)
            {
                ...
                sender.func_197030_a(new TranslationTextComponent("commands.forge.entity.list.single.header", name, info.getLeft()), true);
                ...
                {
                    if (limit-- == 0) break;
                    sender.func_197030_a(new StringTextComponent("  " + e.getValue() + ": " + e.getKey().field_77276_a + ", " + e.getKey().field_77275_b), true);
                }
                return toSort.size();
            }
            else
            {
                ...
                sender.func_197030_a(new TranslationTextComponent("commands.forge.entity.list.multiple.header", count), true);
                info.forEach(e -> sender.func_197030_a(new StringTextComponent("  " + e.getValue() + ": " + e.getKey()), true));
                return info.size();
            }
        }
    }
