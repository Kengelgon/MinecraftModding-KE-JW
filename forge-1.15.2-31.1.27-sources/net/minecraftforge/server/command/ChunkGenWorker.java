...
public class ChunkGenWorker implements IWorker
{
    ...
    {
        return new TranslationTextComponent("commands.forge.gen.start", total, start.func_177958_n(), start.func_177952_p(), dim);
    }
    ...
    {
        ServerWorld world = DimensionManager.getWorld(listener.func_197028_i(), dim, false, false);
        if (world == null)
        {
            world = DimensionManager.initWorld(listener.func_197028_i(), dim);
            if (world == null)
            {
                listener.func_197030_a(new TranslationTextComponent("commands.forge.gen.dim_fail", dim), true);
                queue.clear();
                return false;
            }
        }
        ...
        */
        ...
        if (next != null)
        {
            ...
            {
                listener.func_197030_a(new TranslationTextComponent("commands.forge.gen.progress", total - queue.size(), total), true);
                lastNotification = 0;
                lastNotifcationTime = System.currentTimeMillis();
            }
            ...
            if (!world.func_217354_b(x, z)) { //Chunk is unloaded
                IChunk chunk = world.func_217353_a(x, z, ChunkStatus.field_223226_a_, true);
                if (!chunk.func_201589_g().func_209003_a(ChunkStatus.field_222617_m)) {
                    chunk = world.func_217348_a(x, z, ChunkStatus.field_222617_m);
        ...
        if (queue.size() == 0)
        {
            listener.func_197030_a(new TranslationTextComponent("commands.forge.gen.complete", genned, total, DimensionType.func_212678_a(dim)), true);
